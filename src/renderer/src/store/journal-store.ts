...
export const useJournalStore = create<JournalStore>((set, get) => ({
    ...
    addEntry: (entry) =>
        set((state) => {
            ...
            const newEntries = [...state.journalData.entries, entry];
    ...
    updateEntry: (index, entry) =>
        set((state) => {
            if (!state.journalData) return state;
            const newEntries = [...state.journalData.entries];
    ...
    deleteEntry: (index) =>
        set((state) => {
            if (!state.journalData) return state;
            const newEntries = state.journalData.entries.filter(
                (_, i) => i !== index,
            );
            return {
                journalData: {
    ...
    selectEntry: (entry, index) =>
        set({ selectedEntry: entry, selectedEntryIndex: index }),
    ...
    getEntriesByYear: () => {
        ...
        state.journalData.entries.forEach((entry) => {
    ...
    getFilteredEntries: () => {
        ...
        let entries = state.journalData.entries;
        ...
        if (
            state.searchFilters.sentiment &&
            state.searchFilters.sentiment !== "all"
        ) {
            entries = entries.filter(
                (entry) => entry.sentiment === state.searchFilters.sentiment,
            );
        }
    ...
    getEntryCount: () => {
        const state = get();
        return state.journalData?.entries.length ?? 0;
    },
}));
