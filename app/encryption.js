...
export const getDecryptedText = (text, pwd) => {
    try {
...
export const checkPwdStrength = (pwd) => {
    owasp.config({
        minLength: 8
    });
...
export const encryptFile = (path, outputPath, key, func) => {
    try {
        const salt = crypto.randomBytes(SALT_LENGTH);
        const iv = crypto.randomBytes(IV_LENGTH);
...
export const decryptFile = (path, outputPath, key, func) => {
    try {
        ...
        // Check if file has our new format marker
        // New format: we'll add a magic marker at the very beginning
        // For now, assume legacy format and try that first
        // If it fails, we'll try new format
...
const tryLegacyFormat = (path, outputPath, key, func) => {
    try {
